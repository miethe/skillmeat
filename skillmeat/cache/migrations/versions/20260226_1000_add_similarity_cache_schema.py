...
def _upgrade_fts5() -> None:
    ...
    No-ops on non-SQLite dialects (FTS5 is SQLite-only).
    """
    if not is_sqlite():
        logger.info(
            "Non-SQLite dialect detected — skipping FTS5 %s creation "
            "(FTS5 is SQLite-only)",
            _FTS_TABLE,
        )
        return
...
def downgrade() -> None:
    ...
    """
    # ------------------------------------------------------------------
    # SSO-2.7: Drop FTS5 table and triggers (SQLite-only)
    # ------------------------------------------------------------------
    if is_sqlite():
        ...
        logger.info(
            "Non-SQLite dialect detected — skipping FTS5 %s teardown "
            "(FTS5 is SQLite-only)",
            _FTS_TABLE,
        )
