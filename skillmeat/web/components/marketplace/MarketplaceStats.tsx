...
export function MarketplaceStats({ listings, isLoading }: MarketplaceStatsProps) {
  ...
  const totalArtifacts = listings.reduce(
    (sum, listing) => sum + listing.artifact_count,
    0
  );
  const totalDownloads = listings.reduce(
    (sum, listing) => sum + (listing.downloads || 0),
    0
  );
  const avgRating =
    listings.filter((l) => l.rating !== undefined).reduce(
      (sum, listing) => sum + (listing.rating || 0),
      0
