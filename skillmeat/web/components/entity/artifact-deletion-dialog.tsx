...
export function ArtifactDeletionDialog({
  ...
  const [deleteFromCollection, setDeleteFromCollection] = React.useState(
    context === 'collection'
  );
  ...
  const [selectedProjectPaths, setSelectedProjectPaths] = React.useState<Set<string>>(
    new Set()
  );
        ...
        <DialogHeader className="flex-shrink-0">
          ...
          <DialogDescription asChild>
            <div className="space-y-3">
              <p className="text-destructive font-medium text-sm text-muted-foreground" role="alert">
            ...
            <div className="grid gap-1.5 leading-none flex-1 min-w-0">
              ...
              >
                ...
                {!deploymentsLoading && projectCount > 0 && (
                  <span className="ml-1 text-muted-foreground" aria-label={`${projectCount} project${projectCount !== 1 ? 's' : ''} found`}>
          ...
          {deleteDeployments && deployments.length > 0 && (
            <div className="border border-red-200 dark:border-red-800 rounded-md p-3 sm:p-4 space-y-4 bg-red-50 dark:bg-red-950" role="region" aria-label="Deployment deletion warning">
              ...
              >
                {deploymentsLoading ? (
                  <div className="flex items-center justify-center py-4" role="status" aria-live="polite">
                    <Loader2 className="h-4 w-4 animate-spin mr-2 text-red-600 dark:text-red-400" aria-hidden="true" />
                    <span className="text-sm text-red-700 dark:text-red-300">Loading deployments...</span>
                  </div>
                ) : (
