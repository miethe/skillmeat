 ...
 * Filterable by status, sortable by started date.
 *
 * Next.js 15: params is a Promise — must be awaited.
 * Using 'use client' + useParams() since filtering/sorting require interactivity.
 */
...
export default function WorkflowExecutionsPage() {
  // In Next.js 15 with 'use client', useParams() provides route params.
  ...
  // Infer workflow name from the first execution (denormalized field on API response)
  const workflowName = rows[0]?.workflowName ?? null;
    ...
    <div className="space-y-6 p-6">
      ...
      <nav aria-label="Breadcrumb">
        <Button
          ...
          size="sm"
          asChild
          className="h-8 gap-1.5 text-sm text-muted-foreground hover:text-foreground -ml-2"
        >
      ...
      {/* Main content */}
      <main aria-label="Execution list for this workflow">
        {/* Error state */}
        {isError && (
          <div
            role="alert"
        ...
        {isEmpty && !hasFilters && (
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-24 text-center">
            <PlayCircle
              ...
              aria-hidden="true"
            />
            <h2 className="mb-1 text-base font-semibold">No runs yet</h2>
            <p className="mb-2 max-w-xs text-sm text-muted-foreground">
              This workflow hasn't been executed. Run it to create the first entry.
            </p>
            <p className="font-mono text-xs text-muted-foreground/50">{workflowId}</p>
          </div>
        )}
                ...
                {!isLoading &&
                  rows.map((execution) => (
                    <TableRow
                      ...
                      role="link"
                      aria-label={`View execution ${truncateId(execution.id)}`}
                    >
                      {/* Run ID */}
