...
import type { RefreshModeEnum } from '../models/RefreshModeEnum';
...
export class UserCollectionsService {
   ...
   * @throws ApiError
   */
  public migrateToDefaultCollectionApiV1UserCollectionsMigrateToDefaultPost(): CancelablePromise<
    Record<string, any>
  > {
  ...
  /**
   * Refresh metadata cache for all collections
   * Refresh CollectionArtifact metadata cache across all DB collections. Iterates all collections in the database and refreshes cached metadata from file-based artifacts. This is a bulk operation and may take time for large collections.
   * @returns any Cache refresh completed
   * @throws ApiError
   */
  public refreshAllCollectionsCacheApiV1UserCollectionsRefreshCachePost(): CancelablePromise<
    Record<string, any>
  > {
    return this.httpRequest.request({
      method: 'POST',
      url: '/api/v1/user-collections/refresh-cache',
      errors: {
        500: `Internal server error`,
      },
    });
  ...
  /**
   * Refresh collection metadata cache (DB-backed)
   * Refresh CollectionArtifact metadata cache for a specific DB collection. This is separate from /{collection_id}/refresh, which refreshes file-based collections. This endpoint targets only the DB cache, reading current file-based artifact metadata and updating CollectionArtifact cache rows.
   * @returns any Cache refresh completed successfully
   * @throws ApiError
   */
  public refreshCollectionCacheApiV1UserCollectionsCollectionIdRefreshCachePost({
    collectionId,
  }: {
    collectionId: string;
    ...
    return this.httpRequest.request({
      method: 'POST',
      url: '/api/v1/user-collections/{collection_id}/refresh-cache',
      path: {
        collection_id: collectionId,
      },
      errors: {
        404: `Collection not found in database`,
        422: `Validation Error`,
        500: `Internal server error`,
      },
    });
  ...
  /**
   * Refresh collection artifact metadata
   * Refresh metadata for artifacts in a collection from their upstream GitHub sources. Use mode=check to detect updates without applying changes.
   * @returns any Refresh completed successfully
   * @throws ApiError
   */
  public refreshCollectionApiV1UserCollectionsCollectionIdRefreshPost({
    collectionId,
    requestBody,
    mode,
  }: {
    ...
    /**
     * Override request body mode
     */
    mode?: RefreshModeEnum | null;
  }): CancelablePromise<any> {
    return this.httpRequest.request({
      method: 'POST',
      url: '/api/v1/user-collections/{collection_id}/refresh',
      path: {
        collection_id: collectionId,
      },
      query: {
      ...
      errors: {
        ...
        404: `Collection not found`,
        422: `Validation Error`,
        500: `Internal server error during refresh`,
      },
    });
  }
  /**
   * Get artifact cache statistics
   * Get statistics about artifact metadata cache health including total artifacts, fresh/stale counts, and staleness percentage.
   * @returns any Successful Response
   * @throws ApiError
   */
  public getCacheStatsApiV1UserCollectionsCacheStatsGet({
    ttlSeconds,
    collectionId,
  }: {
    /**
     * Custom TTL in seconds for staleness calculation. Default is 30 minutes (1800 seconds).
     */
    ttlSeconds?: number;
    /**
     * Filter stats to a specific collection ID. If not provided, stats are for all collections.
     */
    collectionId?: string | null;
  }): CancelablePromise<Record<string, any>> {
    return this.httpRequest.request({
      method: 'GET',
      url: '/api/v1/user-collections/cache-stats',
      query: {
