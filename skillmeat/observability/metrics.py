...
mcp_health_checks_total = Counter(
    'skillmeat_mcp_health_checks_total',
    'Total MCP health checks',
    ['server', 'status']
)

mcp_deployments_total = Counter(
    'skillmeat_mcp_deployments_total',
    'Total MCP deployments',
    ['server', 'status']
)
...
mcp_operation_duration = Histogram(
    ...
    ['operation'],
    buckets=[.1, .5, 1.0, 2.0, 5.0, 10.0, 30.0]
)
...
artifact_operation_duration = Histogram(
    ...
    ['operation', 'type'],
    buckets=[.01, .05, .1, .5, 1.0, 5.0, 10.0]
)
...
cache_hits_total = Counter(
    'skillmeat_cache_hits_total',
    'Total cache hits',
    ['cache_name']
)

cache_misses_total = Counter(
    'skillmeat_cache_misses_total',
    'Total cache misses',
    ['cache_name']
)
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if metric_name == "bundle":
                    bundle_operation_duration.labels(operation=operation).observe(duration)
