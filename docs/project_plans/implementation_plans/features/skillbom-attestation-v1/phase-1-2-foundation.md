...
| 2.8 | Pydantic schemas in `skillmeat/api/schemas/bom.py` | Define `BomSchema`, `ArtifactEntrySchema`, `BomMetadataSchema`, `AttestationSchema`, `HistoryEventSchema`. All cover all 13+ artifact types. | Schemas validate correct structure; from_orm mode works for all fields; schema version and artifact types validated; no required fields missing | 3 | Pending |
