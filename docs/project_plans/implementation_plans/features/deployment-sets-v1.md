...
| DS-002 | DeploymentSet CRUD repo | Implement `DeploymentSetRepository` in `skillmeat/cache/repositories.py` with `create`, `get`, `list` (paginated, filterable by name/tag), `update`, `delete`. Delete operation must remove parent references where deleted set appears as `member_set_id` (FR-10) before deleting row. Add `owner_id` persistence and scoping hooks for authenticated mode with single-user fallback. Return ORM objects; DTO mapping is service responsibility. | Unit tests cover CRUD, owner-scoped reads/writes, and FR-10 delete behavior (parent references removed). `list` accepts `name`, `tag`, `limit`, `offset`. | 2 pts | `python-backend-engineer` | DS-001 |
...
| DS-006 | Batch deploy service | Implement `DeploymentSetService.batch_deploy(set_id, project_id, profile_id) -> list[DeployResultDTO]`. Call `resolve(set_id)` to get UUID list, then map each `artifact_uuid` to deploy contract inputs (`artifact_id`, `artifact_name`, `artifact_type`) and resolve `project_id` to `project_path` before invoking deploy service. Catch per-artifact exceptions; never abort loop. Return `list[{artifact_uuid, status: "success"|"skip"|"error", error: str|None}]`. Emit structured logs for success/failure and warning logs for missing-member resolution paths. | Unit tests with mocked deploy service: mixed-result partial failure, all-success, mapping failure path, missing member warning path, and deploy exception -> `"error"` with message. | 2 pts | `python-backend-engineer` | DS-005 |
...
- [ ] Batch deploy adapter validated (`artifact_uuid` -> deploy request inputs + project path resolution)
...
| DS-008 | REST endpoints + router registration | Create `skillmeat/api/routers/deployment_sets.py` with all 11 endpoints per PRD §13 API surface. Register router in `skillmeat/api/server.py` with API prefix wiring. Map `DeploymentSetCycleError` → HTTP 422; `DeploymentSetResolutionError` → HTTP 422; `not found` → HTTP 404. Enforce owner scoping from authenticated claims when auth is enabled (single-user fallback when disabled). Clone endpoint (`POST /{id}/clone`) duplicates set + all direct member rows, appends " (copy)" to name. | `pytest` integration tests (FastAPI `TestClient`) pass for all 11 endpoints (happy + error paths). HTTP 422 returned for circular-ref attempt. Owner-scope isolation verified in auth-enabled test mode. | 3 pts | `python-backend-engineer` | DS-007 |
...
| Task ID | Task Name | Description | Acceptance Criteria | Estimate | Subagent(s) | Dependencies |
|---------|-----------|-------------|-------------------|----------|-------------|--------------|
| DS-T01 | Integration tests: circular-ref + batch deploy + delete semantics | Extend API integration tests (FastAPI `TestClient`): (1) create set A → B then attempt B → A, assert 422; (2) batch deploy 3-level nested set to mock project, assert all UUIDs appear exactly once and adapter mapping is invoked; (3) delete member-set referenced by parent and assert inbound parent references are removed (FR-10); (4) clone test: modify clone, assert source unchanged. | All scenarios pass repeatedly without flakiness. | 1 pt | `python-backend-engineer` | DS-008 |
...
| DS-T04 | Documentation + hook exports + feature flag wiring | Confirm `skillmeat/web/hooks/index.ts` exports all new hooks. Verify OpenAPI spec auto-generates correctly from FastAPI router (run `skillmeat web dev --api-only` and check `/docs`). Implement and validate `deployment_sets_enabled` wiring in config + frontend nav/page gating. | All hooks exported. OpenAPI `/docs` shows all 11 endpoints. Feature flag toggles nav/page visibility deterministically. | 1 pt | `documentation-writer`, `api-documenter` | DS-013 |
...
| UUID-based resolver output cannot call current deploy contract directly | High | Medium | Add explicit adapter in DS-006 (`artifact_uuid` -> `artifact_id/name/type`, `project_id` -> `project_path`) and test mapping failures |
