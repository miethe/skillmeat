...
class TestListCommand:
    ...
    def test_list_filter_by_type_skill(self, isolated_cli_runner, sample_skill_dir, sample_command_file):
    ...
    def test_list_filter_by_type_command(self, isolated_cli_runner, sample_skill_dir, sample_command_file):
...
class TestShowCommand:
    ...
    def test_show_ambiguous_name(self, isolated_cli_runner, tmp_path):
        ...
        runner.invoke(main, ['init'])
        runner.invoke(main, ['add', 'skill', str(skill_dir), '--dangerously-skip-permissions'])
        runner.invoke(main, ['add', 'command', str(command_file), '--name', 'shared-name', '--dangerously-skip-permissions'])
...
class TestRemoveCommand:
    ...
    def test_remove_existing_artifact(self, isolated_cli_runner, sample_skill_dir, temp_home):
        """Test removing an existing artifact."""
        runner = isolated_cli_runner

        # Initialize and add artifact
        runner.invoke(main, ['init'])
        add_result = runner.invoke(main, ['add', 'skill', str(sample_skill_dir), '--dangerously-skip-permissions'])
        ...
        assert "test-skill" not in list_result.output or "No artifacts found" in list_result.output
    ...
    def test_remove_with_keep_files(self, isolated_cli_runner, sample_skill_dir, temp_home):
    ...
    def test_remove_with_type_specification(self, isolated_cli_runner, sample_skill_dir):
    ...
    def test_remove_and_verify_files_deleted(self, isolated_cli_runner, sample_skill_dir, temp_home):
