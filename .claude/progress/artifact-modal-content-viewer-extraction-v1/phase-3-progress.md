...
tasks:
- id: EXT-301
  description: ContentPane extraction — extract the top-level ContentPane component
    and its direct children into the package; verify exported types and props surface
  status: completed
  story_points: 3
  ...
  - EXT-204
- id: EXT-302
  description: SplitPreview and MarkdownEditor extraction — extract SplitPreview and
    MarkdownEditor surfaces into the package on top of the extracted ContentPane
  status: completed
  story_points: 2
  assigned_to: ui-engineer-enhanced
  dependencies:
  - EXT-301
- id: EXT-303
  description: Hook adapter abstraction — introduce adapter interfaces so consumers
    can inject their own data-fetching hooks without coupling to SkillMeat's API layer
  status: completed
  story_points: 2
  ...
  - EXT-301
- id: EXT-304
  description: Performance guardrails — add bundle-size and render-count assertions
    to CI; verify the extracted package does not regress on Core Web Vitals indicators
  status: completed
  story_points: 1
