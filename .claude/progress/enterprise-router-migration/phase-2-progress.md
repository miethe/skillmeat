...
tasks:
  - id: "TASK-2.1"
    description: "Migrate artifacts.py utilities: resolve_collection_name (~L369), _find_artifact_in_collections (~L465), build_version_graph (~L679). Replace manager reads with CollectionRepoDep/ArtifactRepoDep/DbArtifactHistoryRepoDep. Gate discover() and discover_in_project() with edition check (501)."
    status: "pending"
  ...
  - id: "TASK-2.2"
    description: "Migrate marketplace_sources.py: get_collection_artifact_keys helper (~L612) uses ArtifactManager.enumerate_all(). Replace with ArtifactRepoDep.list_by_collection() or equivalent DB query. Cascades to ~6 endpoints."
    status: "pending"
    assigned_to: ["python-backend-engineer"]
    dependencies: ["TASK-1.2"]
    estimated_effort: "3 pts"
  ...
  - id: "TASK-2.3"
    description: "Migrate match.py: single endpoint (~L65) uses artifact_mgr.match(). Replace with ArtifactRepoDep.search() or simple DB name-matching query. Add search method to repository if missing."
    status: "pending"
    assigned_to: ["python-backend-engineer"]
    dependencies: ["TASK-1.2"]
...
success_criteria:
  - { id: "SC-1", description: "artifacts.py utilities work with repo DI in enterprise", status: "pending" }
  - { id: "SC-2", description: "Discovery endpoints return 501 in enterprise", status: "pending" }
  - { id: "SC-3", description: "marketplace_sources helper returns correct data from DB", status: "pending" }
  - { id: "SC-4", description: "match endpoint returns results from DB in enterprise", status: "pending" }
