...
tasks:
  - id: "TASK-3.1"
    description: "Migrate tags.py: ~4 READ ops use CollectionManagerDep for tag lookups -> TagRepoDep. ~2 WRITE ops: wrap FS write-through in edition check (skip in enterprise)."
    status: "pending"
  ...
  - id: "TASK-3.2"
    description: "Migrate user_collections.py: ~3 READ ops use managers -> DbUserCollectionRepoDep/DbCollectionArtifactRepoDep. ~5 WRITE ops: edition-conditional FS write-through."
    status: "pending"
    assigned_to: ["python-backend-engineer"]
    dependencies: ["TASK-1.2"]
    estimated_effort: "2 pts"
  ...
  - id: "TASK-3.3"
    description: "Migrate deployment_sets.py: audit and replace any CollectionManagerDep/ArtifactManagerDep reads with DeploymentRepoDep or appropriate repo."
    status: "pending"
    assigned_to: ["python-backend-engineer"]
    dependencies: ["TASK-1.2"]
  ...
  - id: "TASK-3.5"
    description: "Clean up health.py: remove CollectionManagerDep from readiness check signature (already edition-aware in detailed health). Ensure no FS access in enterprise."
    status: "pending"
    assigned_to: ["python-backend-engineer"]
    dependencies: ["TASK-1.2"]
  ...
  - id: "TASK-3.6"
    description: "Migrate mcp.py: ~3 READ ops -> ArtifactRepoDep. ~3 WRITE ops: gate behind edition check (MCP is FS-native, return 501 for write ops in enterprise)."
    status: "pending"
    assigned_to: ["python-backend-engineer"]
    dependencies: ["TASK-1.2"]
    estimated_effort: "0.5 pts"
...
parallelization:
  batch_1: ["TASK-3.1", "TASK-3.2", "TASK-3.3", "TASK-3.4", "TASK-3.5", "TASK-3.6"]
...
success_criteria:
  - { id: "SC-1", description: "Tag CRUD works in enterprise", status: "pending" }
  - { id: "SC-2", description: "Collection operations work in enterprise", status: "pending" }
  - { id: "SC-3", description: "Health endpoints dont trigger FS access in enterprise", status: "pending" }
  - { id: "SC-4", description: "No CollectionManagerDep/ArtifactManagerDep in READ paths", status: "pending" }
