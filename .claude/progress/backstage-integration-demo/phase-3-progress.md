...
tasks:
  - id: "TASK-3.1"
    description: "Create fin-serv-compliance composite seed data script — composite type 'stack' with three member artifacts: CLAUDE.md (ProjectConfig), agent:db-architect, mcp:internal-db-explorer"
    status: "pending"
...
blockers:
  - id: "BLOCKER-P3-001"
    ...
    severity: "high"
    blocking: ["TASK-3.2", "TASK-3.3"]
    resolution: "Phase 1 (TASK-1.5) and Phase 2 (TASK-2.4) completion"
    created: "2026-03-03"

success_criteria:
  - { id: "SC-1", description: "docker-compose demo-up starts all services healthy on macOS and Linux", status: "pending" }
  - { id: "SC-2", description: "Backstage template creates GitHub repo with .claude/ directory", status: "pending" }
  - { id: "SC-3", description: "Agent in cloned repo uses injected CLAUDE.md constraints without manual setup", status: "pending" }
  - { id: "SC-4", description: "DEMO_GUIDE.md is reproducible by a new operator", status: "pending" }
