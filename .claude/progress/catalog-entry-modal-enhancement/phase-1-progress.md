...
- id: TASK-1.1
  ...
  dependencies: []
  acceptance: Method returns list of file paths, types, sizes from GitHub Tree API.
    Handles recursive trees.
  files:
...
- id: TASK-1.2
  ...
  - python-backend-engineer
  dependencies: []
  acceptance: Method returns file content (base64-decoded), encoding, size, SHA from
    GitHub Contents API.
  files:
  - skillmeat/core/marketplace/github_scanner.py
  commit: 31224e8
...
- id: TASK-1.5
  ...
  - python-backend-engineer
  dependencies: []
  acceptance: 'LRU cache with max 1000 entries. TTL: 1hr (trees), 2hr (contents).
    Keys: tree:{source_id}:{artifact_path}:{sha}.'
  files:
  - skillmeat/api/utils/github_cache.py
  commit: 38d4c31
  completed_at: '2025-12-28'
- id: TASK-1.3
  ...
  - TASK-1.1
  - TASK-1.5
  acceptance: GET /marketplace/sources/{id}/artifacts/{path}/files returns cached
    file tree DTO.
  files:
...
- id: TASK-1.4
  ...
  - TASK-1.2
  - TASK-1.5
  acceptance: GET /marketplace/sources/{id}/artifacts/{path}/files/{file_path} returns
    cached content DTO.
  files:
  - skillmeat/api/routers/marketplace_sources.py
  - skillmeat/api/schemas/marketplace.py
...
- id: TASK-1.6
  ...
  - TASK-1.3
  - TASK-1.4
  acceptance: 'Detect GitHub HTTP 403 with X-RateLimit-Remaining: 0. Return HTTP 429
...
- id: TASK-1.7
  ...
  - TASK-1.4
  - TASK-1.6
  acceptance: Test happy path, 404 errors, rate limits, cache hits/misses. Coverage
    >80%.
  files:
  - tests/api/test_marketplace_sources.py
  commit: e73d8f5
  completed_at: '2025-12-28'
- id: TASK-1.8
  ...
  - TASK-1.3
  - TASK-1.4
  acceptance: Add /files and /files/{file_path} endpoints to OpenAPI schema with request/response
    models.
  files:
  - skillmeat/api/server.py
  commit: auto-generated
  completed_at: '2025-12-28'
  notes: OpenAPI auto-generated by FastAPI from router and schema definitions
parallelization:
  batch_1:
  - TASK-1.1
  - TASK-1.2
...
- [x] GitHubScanner can fetch file trees and individual file contents
- [x] Backend cache reduces GitHub API calls by 70%+ for repeated requests
- [x] Rate limit errors (HTTP 403) are detected and handled gracefully
- [x] All endpoints return proper DTOs (Pydantic models)
- [x] Unit tests achieve >80% coverage for new code (27 tests passing)
